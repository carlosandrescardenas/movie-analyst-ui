...
var backendHost = process.env.BACK_HOST || '192.168.55.6';
...
app.get('/movies', function(req, res){
  request
    .get('http://'+backendHost+':8085/movies')
...
app.get('/authors', function(req, res){
  request
    .get('http://'+backendHost+':8085/reviewers')
...
app.get('/publications', function(req, res){
  request
    .get('http://'+backendHost+':8085/publications')
    .end(function(err, data) {
      if(data.status == 403){
        res.send(403, '403 Forbidden');
      } else {
...
app.get('/pending', function(req, res){
  request
    .get('http://'+backendHost+':8085/pending')
    .end(function(err, data) {
      if(data.status == 403){
        res.send(403, '403 Forbidden');
      }
